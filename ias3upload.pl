...
sub initConfig {
    ...
    if ( -f $cfg ) {
        unless (
            confirm(
                "\nOh, you already have ~/.ias3cfg. Are you sure you want to overwrite it? (y/N):",
                0
            )
            )
        {
    ...
    if (confirm(
            "\nI can get your keys from IA web site for you. Want to try? (y/N):",
            0
        )
        )
    {
        ...
        my ( $username, $password );
        {
            ...
            ( $accessKey, $secretKey )
                = getKeysFromWeb( $username, $password );
    ...
    unless ( $accessKey && $secretKey ) {
            ...
            "get them from http://www.archive.org/account/s3.php.\n\n";
        {
            ...
            unless ( checkKey($accessKey) ) {
                print
                    "Bad key - it should be letters and digits, length 16.\n";
                redo;
            }
        }
        {
...
sub getKeysFromWeb {
    ...
    if ( $res->is_success ) {
        ...
        my @keys    = $content
            =~ /Your S3 access key: ([[:alnum:]]{16}).*Your S3 secret key: ([[:alnum:]]{16})/s;
        if (@keys) {
            warn "okay, found keys successfully.\n";
        }
        else {
            warn
                "sorry, couldn't find keys on the page, probably due to page layout change. Please get keys manually.\n";
        }
        return @keys;
    }
    else {
        warn
            "sorry, couldn't load S3 keys page (possibly problem with network or Internet Archive server). Please get keys manually.\n";
        return ();
    }
}
...
sub metadataHeaders {
    ...
    if ( ref $v eq 'ARRAY' ) {
        if ( $#{$v} == 0 ) {
            ...
            my $i = 1;
            return map( (
                    sprintf( 'x-archive-meta%02d-%s', $i++, $h ),
                    encodeHeaderValue($_)
                ),
                @$v );
        }
